...
//
// UPDATE: “leave running” mode + robust best-L search (as provided by user).
//
// PERFORMANCE / RELIABILITY UPGRADES IMPLEMENTED (second variant, high ROI):
//   (1) Remove inner OpenMP from triangle-triangle loops (too fine-grained).
//   (2) Avoid per-triangle temporary Vec2[3] arrays in the hot loop.
//   (3) Early exit on first intersecting triangle-pair (first-hit) in both:
//         - overlap_pair_bool()   (boolean collision test)
//         - overlap_pair_penalty() (penalty depth^2 for first hit)
//   (4) Add STRICT feasibility check for accept/reject decisions in outer loops:
//         - Uses overlap_pair_bool() with grid neighbor enumeration to detect ANY overlap.
//         - Uses AABB-vs-square to detect ANY out-of-bounds.
//       This is more reliable than comparing a soft penalty sum to a tiny tolerance.
//
// Compile:
...
// ---------------- SAT for triangle-triangle (FAST: no arrays, early exit) ----------------

static inline void proj3_pts(const Vec2 p0, const Vec2 p1, const Vec2 p2,
                             double ax, double ay, double *mn, double *mx)
{
    const double v0 = p0.x * ax + p0.y * ay;
    const double v1 = p1.x * ax + p1.y * ay;
    const double v2 = p2.x * ax + p2.y * ay;
...
// Returns 1 if triangles intersect; outputs min SAT overlap depth on tested axes.
static inline int tri_sat_penetration_pts(
    const Vec2 A0, const Vec2 A1, const Vec2 A2,
    ...
    double *depth_out)
{
    ...
    for (int pass = 0; pass < 2; pass++) {
        const Vec2 T0 = (pass == 0) ? A0 : B0;
        const Vec2 T1 = (pass == 0) ? A1 : B1;
        const Vec2 T2 = (pass == 0) ? A2 : B2;

        for (int e = 0; e < 3; e++) {
            Vec2 p0, p1;
            if (e == 0) { p0 = T0; p1 = T1; }
            else if (e == 1) { p0 = T1; p1 = T2; }
            else { p0 = T2; p1 = T0; }
            ...
            proj3_pts(A0, A1, A2, ax, ay, &amin, &amax);
            proj3_pts(B0, B1, B2, ax, ay, &bmin, &bmax);

            const double o = fmin(amax, bmax) - fmax(amin, bmin);
            if (o <= 0.0) return 0;
...
// ---------------- Pair overlap tests (boolean + penalty) ----------------
...
// Boolean overlap: return 1 if ANY triangle-triangle intersects, else 0.
// High ROI: early exit on first hit.
static inline int overlap_pair_bool(const State *s, int i, int j) {
    if (!aabb_overlap(&s->aabb[i], &s->aabb[j])) return 0;
    if (bounding_circle_reject(s, i, j)) return 0;
    ...
    for (int ta = 0; ta < NTRI; ta++) {
        ...
        const int a0i = TRIS[ta].a;
        const int a1i = TRIS[ta].b;
        const int a2i = TRIS[ta].c;
        const Vec2 A0 = wi[a0i];
        const Vec2 A1 = wi[a1i];
        const Vec2 A2 = wi[a2i];

        for (int tb = 0; tb < NTRI; tb++) {
            if (!aabb_overlap(aTa, &aj[tb])) continue;

            const int b0i = TRIS[tb].a;
            const int b1i = TRIS[tb].b;
            const int b2i = TRIS[tb].c;
            const Vec2 B0 = wj[b0i];
            const Vec2 B1 = wj[b1i];
            const Vec2 B2 = wj[b2i];

            double depth = 0.0;
            if (tri_sat_penetration_pts(A0, A1, A2, B0, B1, B2, &depth)) {
                return 1;
            }
        }
    }
...
// Penalty overlap: return depth^2 for FIRST hit, else 0.
// High ROI: early exit on first hit.
...
// Sum overlap penalties for k against neighbors (for incremental SA energy updates).
...
// Full totals (penalty sums) used by SA energy.
...
// ---------------- STRICT feasibility (boolean, high ROI for decision logic) ----------------

// Return 1 if ANY polygon is out of bounds (using AABB vs square).
static int any_outside_strict(const State *s, double tol_abs) {
    const double half = 0.5 * s->L;
    for (int i = 0; i < s->N; i++) {
        const AABB *b = &s->aabb[i];
        if (b->minx < -half - tol_abs) return 1;
        if (b->maxx >  half + tol_abs) return 1;
        if (b->miny < -half - tol_abs) return 1;
        if (b->maxy >  half + tol_abs) return 1;
    }
    return 0;
}

// Return 1 if ANY overlap exists (using grid neighbor enumeration + overlap_pair_bool()).
static int any_overlap_strict_grid(const State *s) {
    const Grid *g = &s->grid;
    int R = grid_R_cells(s);
    ...
    for (int i = 0; i < s->N; i++) {
        ...
        for (int dy = -R; dy <= R; dy++) {
            ...
            for (int dx = -R; dx <= R; dx++) {
                ...
                for (int j = g->head[c]; j != -1; j = g->next[j]) {
                    if (j <= i) continue;
                    if (overlap_pair_bool(s, i, j)) return 1;
                }
            }
        }
    }
...
static inline int is_feasible_strict(const State *s, double out_tol_abs) {
    if (any_outside_strict(s, out_tol_abs)) return 0;
    if (any_overlap_strict_grid(s)) return 0;
    return 1;
}
...
int main(int argc, char **argv) {
    ...
    // ---- Outer loop controls ----
    const double out_tol_abs = 1e-12;   // strict boundary tolerance (absolute)
    ...
    int feas0_flag = is_feasible_strict(&s, out_tol_abs) && !guaranteed_infeasible_by_area(s.L, area_lb);

    printf("Initial result: L=%.12g feas=%.3e (%s)\n",
           s.L, feas0, feas0_flag ? "FEASIBLE(strict)" : "INFEASIBLE(strict)");
    ...
    if (feas0_flag) {
        ...
        for (int k = 0; k < bracket_max_steps; k++) {
            ...
            int feasible_flag = is_feasible_strict(&s, out_tol_abs) && !guaranteed_infeasible_by_area(s.L, area_lb);

            printf("Bracket check: L=%.12g feas=%.3e (%s)\n",
                   s.L, feas, feasible_flag ? "FEASIBLE(strict)" : "INFEASIBLE(strict)");
    ...
    printf("L=%.12g ov=%.6e out=%.6e feas=%.6e  strict=%s\n",
           s.L, final_tot.overlap_total, final_tot.out_total, final_feas,
           is_feasible_strict(&s, out_tol_abs) ? "YES" : "NO");
    ...
    while (1) {
        ...
        if ((tnow - last_ckpt_time) >= cfg.checkpoint_every_sec) {
            ...
            printf("[ckpt] t=%.0fs bestL=%.12g bestFeas=%.3e eps=%.3g strict=%s\n",
        ...
        // Restore candidate best from the SA attempt, then check strict feasibility.
        for (int i = 0; i < N; i++) { s.cx[i] = tmp_cx[i]; s.cy[i] = tmp_cy[i]; s.th[i] = tmp_th[i]; }
        update_all(&s);
        rebuild_grid(&s);

        int feasible_flag = is_feasible_strict(&s, out_tol_abs) && !guaranteed_infeasible_by_area(L_try, area_lb);
        ...
        if (feasible_flag) {
            ...
            printf("[improve] attempt=%d bestL=%.12g feas=%.3e eps=%.3g strict=YES\n",
    ...
    printf("bestL=%.12g ov=%.6e out=%.6e feas=%.6e strict=%s\n",
